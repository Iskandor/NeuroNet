...
CACLA::CACLA(NeuralNetwork *p_actor, NeuralNetwork *p_critic) : ActorCritic(p_actor, p_critic) {
  _criticLearning = new TDLambda(p_critic, 0.9, 0.8);
...
void CACLA::run() {
  ...
  //VectorXd action1 = VectorXd::Zero(_actor->getOutput()->size());
  double reward = 0;
  ...
  //getAction(&state1, &action1);
  reward = _environment->getReward();

  CACLAActor* actorLearning = dynamic_cast<CACLAActor *>(_actorLearning);
  TDLambda* criticLearning = dynamic_cast<TDLambda *>(_criticLearning);

  double tdError = criticLearning->train(&state0, &state1, reward);
  if (tdError > 0) {
